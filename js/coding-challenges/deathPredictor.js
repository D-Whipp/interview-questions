...
const getUserAge = () => {
    ...
    if (isNaN(userAge)) {
        alert('PLEASE ENTER A NUMBER YOU DULT!');
        getUserAge();
    }
    return parseInt(userAge);
};

const getYearsLeft = (userAge) => {
...
const calculateDays = (yearsLeft) => {
    daysLeft = 365 * yearsLeft;

    return daysLeft;
};

const calculateWeeks = (yearsLeft) => {
    weeksLeft = 52 * yearsLeft;

    return weeksLeft;
};

const calculateMonths = (yearsLeft) => {
...
getUserAge();
getYearsLeft(userAge);
calculateDays(yearsLeft);
calculateWeeks(yearsLeft);
calculateMonths(yearsLeft);
